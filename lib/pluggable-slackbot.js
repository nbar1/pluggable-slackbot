...
function pluggableSlackbot(config) {
	 ...
	 * @returns {null}
	 */
	this.sendMessage = function(channel, message, options) {
	...
	this.importPlugins = function() {
		...
		fs.readdirSync(this.config.pluginPath).forEach(function(file) {
			...
			if (_this.plugins[pluginId].config.enabled === true && _this.plugins[pluginId]['initialize'] !== undefined) {
				_this.plugins[pluginId]['initialize'].call(_this.plugins[pluginId]);
			}
		});
